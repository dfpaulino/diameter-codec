...
@AvpRegister(avpCode = 1047,avpBuilderMethod = "byteToAvp")
public class PreEmptionCapability extends Avp<Integer> {
    public static int avpCode = 1047;
    public static byte flags = (byte) 0xc0;

    public PreEmptionCapability(AvpHeader header, byte[] buffer, int position) {
        super(header, buffer, position);
    }

    public PreEmptionCapability(Integer data) {
        super(data);
    }

    public static AvpBuilder byteToAvp(){
        return new AvpBuilder(PreEmptionCapability::new);
    }
