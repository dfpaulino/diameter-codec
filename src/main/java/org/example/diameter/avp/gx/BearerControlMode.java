...
@AvpRegister(avpCode = 1023,avpBuilderMethod = "byteToAvp")
public class BearerControlMode extends Avp<Integer> {
    public static int avpCode = 1023;
    public static byte flags = (byte) 0xc0;

    public BearerControlMode(AvpHeader header, byte[] buffer, int position) {
        super(header, buffer, position);
    }

    public BearerControlMode(Integer data) {
        super(data);
    }

    public static AvpBuilder byteToAvp(){
        return new AvpBuilder(BearerControlMode::new);
    }
    @Override
